...
function AmazonScraperTab() {
  ...
  return (
    <div className="container">
      <h2>Amazon Scraper - Single ASIN</h2>
      
      <div className="input-container">
        <label htmlFor="api-key-input" className="input-label">API Key:</label>
        <input 
          id="api-key-input"
          type="text" 
          value={apiKey} 
      ...
      <div className="input-container">
        <label htmlFor="single-asin-input" className="input-label">ASIN:</label>
        <input 
          id="single-asin-input"
          type="text" 
          value={singleASIN} 
      ...
      <div className="action-buttons">
        <button onClick={handleStart} disabled={isLoading} className="button">
          {isLoading ? 'Đang xử lý...' : 'Bắt Đầu'}
        </button>
        <button onClick={handleStop} className="button">Dừng</button>
        <button onClick={handleDownload} disabled={!productInfo} className="button">Tải Kết Quả</button>
      </div>
      ...
      {productInfo && (
        <div className="download-section">
          ...
          <table className="image-table">
            ...
            <tbody>
              <tr>
                <td data-label="Ảnh"><img src={productInfo.mainImages[0]} alt="Main" className="thumbnail" /></td>
                <td data-label="ASIN | Variant">{`${productInfo.mainImageAsin}`}</td>
                <td data-label="Tải ảnh">
                  <a 
            ...
            <tbody>
              {(() => {
                ...
                return Object.entries(productInfo.hiResImages).map(([asin, urls]) => (
                  <tr key={asin}>
                    <td data-label="Ảnh"><img src={urls[0]} alt={`Variant ${asin}`} className="thumbnail" /></td>
                    <td data-label="ASIN | Variant">{`${asin}`}</td>
                    <td data-label="Tải ảnh">
